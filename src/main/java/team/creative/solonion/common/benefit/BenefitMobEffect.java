...
public class BenefitMobEffect extends Benefit<MobEffect> {
    ...
    public BenefitMobEffect(MobEffect effect, double value) {
        this(BuiltInRegistries.MOB_EFFECT.getKey(effect), value);
    }
    ...
    public static class BenefitTypeMobEffect extends BenefitType<BenefitMobEffect, Object2IntMap<Holder<MobEffect>>, AppliedMobEffects> {
        ...
        public void onEffectRemove(MobEffectEvent.Remove event) {
            if (event.getEntity() instanceof Player player) {
                ...
                if (applied != null && !applied.reseting && applied.contains(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(event.getEffect())))
        ...
        public void loadApplied(AppliedMobEffects applied, Tag nbt) {
            if (nbt instanceof ListTag list)
                for (int i = 0; i < list.size(); i++) {
                    Reference<MobEffect> mob = BuiltInRegistries.MOB_EFFECT.getHolder(ResourceKey.create(Registries.MOB_EFFECT, new ResourceLocation(list.getString(i)))).get();
                    if (mob != null)
                        applied.add(mob);
                }
        }
        ...
        public boolean apply(Player player, AppliedMobEffects applied, @Nullable Object2IntMap<Holder<MobEffect>> stack) {
            ...
            if (stack != null) {
                for (var entry : stack.object2IntEntrySet()) {
                    var in = new MobEffectInstance(entry.getKey().get(), -1, entry.getIntValue(), false, false);
